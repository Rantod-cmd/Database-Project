...
export default function ProvincialData() {
    ...
    return (
        <div className="pb-24 px-6 max-w-7xl mx-auto min-h-screen pt-12">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
                <div className="flex flex-wrap items-center justify-between gap-6 p-8 bg-white border border-slate-100 rounded-[40px] shadow-sm">
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => {
                                ...
                                if (totalPages > 7) {
                                    if (p !== 1 && p !== totalPages && (p < page - 1 || p > page + 1)) {
                                        if (p === page - 2 || p === page + 2) return <span key={`dots-${p}`} className="text-slate-300">...</span>;
                                        return null;
                                    }
                                }
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-12 h-12 rounded-2xl font-black text-sm transition-all ${page === p
